...
class GridWorldEnv(gym.Env):
    """
    Multi-Agent Grid World Environment (Hardcore Mode).
    
    Changes:
    - Action Space: 4 (No Stay)
    - Revisit Penalty: -0.1
    ...
    def __init__(self, 
        ...
        if self.obs_mode == 'grid':
            ...
            self.observation_space = spaces.Dict({
                'local': spaces.Box(low=0, high=1, shape=(3, grid_size, grid_size), dtype=np.float32),
                'global': spaces.Box(low=0, high=1, shape=(1, 10, 10), dtype=np.float32)
            })
        elif self.obs_mode == 'target':
            self.observation_space = spaces.Dict({
                'local': spaces.Box(low=0, high=1, shape=(3, grid_size, grid_size), dtype=np.float32),
                'target': spaces.Box(low=-1, high=1, shape=(2,), dtype=np.float32)
            })
        elif self.obs_mode == 'mixed':
             self.observation_space = spaces.Dict({
                'image': spaces.Box(low=0, high=1, shape=(3, grid_size, grid_size), dtype=np.float32),
                'vector': spaces.Box(low=0, high=1, shape=(2,), dtype=np.float32)
            })
    ...
    def step(self, actions):
        ...
        for i, action in actions.items():
            # No living cost, penalty comes from revisit/collision
            agent = self.agents[i]
            if not agent['active']:
                new_positions[i] = agent['pos']
                continue
        ...
        for i, pos in final_positions.items():
            ...
            if self.visited_map[pos[0], pos[1]] == 0:
                rewards[i] += 1.0 # New Tile Reward
                self.visited_map[pos[0], pos[1]] = 1
                total_new_coverage += 1
            else:
                rewards[i] -= 0.1 # Revisit Penalty
    ...
    def _get_observations(self):
        ...
        if self.obs_mode == 'global':
             global_map = self._get_global_map_obs()
        ...
        for i in range(self.n_agents):
            if self.obs_mode == 'grid':
                ...
                obs[i] = {
                    'local': self._get_grid_obs(i),
                    'target': self._get_target_obs(i)
                }
            elif self.obs_mode == 'global':
                obs[i] = {
                    'local': self._get_grid_obs(i),
                ...
                obs[i] = {
                    'image': self._get_grid_obs(i),
                    'vector': self._get_gps_obs(i)
                }
            elif self.obs_mode == 'vector':
                obs[i] = self._get_vector_obs(i)
        return obs

    def _get_target_obs(self, agent_id):
        unvisited = np.argwhere(self.visited_map == 0)
        if len(unvisited) == 0:
            return np.array([0.0, 0.0], dtype=np.float32)
        agent_pos = self.agents[agent_id]['pos']
        dists = np.abs(unvisited - agent_pos).sum(axis=1)
        min_idx = np.argmin(dists)
        nearest_target = unvisited[min_idx]
        vec = nearest_target - agent_pos
        norm = np.linalg.norm(vec)
        if norm > 0:
            vec = vec / norm
        return vec.astype(np.float32)

    def _get_global_map_obs(self):
        if self.width == 50 and self.height == 50:
             return self.visited_map.reshape(10, 5, 10, 5).mean(axis=(1, 3))[np.newaxis, ...].astype(np.float32)
        return np.zeros((1, 10, 10), dtype=np.float32)

    def _get_gps_obs(self, agent_id):
    ...
    def _get_vector_obs(self, agent_id):
    ...
    def _render_pygame(self):
        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                elif self.visited_map[x, y] == 1:
                     pygame.draw.rect(self.window, (200, 255, 200), 
                                     (x*self.cell_size, y*self.cell_size, self.cell_size, self.cell_size))
        for i, agent in self.agents.items():
            ax, ay = agent['pos']
            pygame.draw.circle(self.window, (255, 0, 0),
                               (int((ax + 0.5) * self.cell_size), int((ay + 0.5) * self.cell_size)),
